...
import org.apache.skywalking.oap.server.storage.plugin.kafka.elasticsearch.IndicesMetadataCache;
...
public class MetricsEsDAO extends EsDAO implements IMetricsDAO {
    ...
    public UpdateRequest prepareBatchUpdate(Model model, Metrics metrics) {
        ...
        String id = IndexController.INSTANCE.generateDocId(model, metrics.id());
        //sender to kafka
        KafkaSenderHandler.getInstance().sender(new DataWrapper(builder, IndexController.INSTANCE.getTableName(model), true, id));
